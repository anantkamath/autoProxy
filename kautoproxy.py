...
from PyKDE4.kdecore import * #Used to write KDE system wide proxy settings to kioslaverc
import time #Sleep
...
def save(settings):
	#Start changing from this line from for gnome (gconf):
	config = KConfig('kioslaverc',  KConfig.SimpleConfig) #Loads the kde network proxy config file, kioslaverc
	config.sync()
	group = config.group("Proxy Settings")
	group.writeEntry( "ftpProxy", settings.proxy_ip+(' ' if(settings.proxy_ip!='') else '')+ settings.proxy_port )
	group.writeEntry( "httpProxy", settings.proxy_ip+(' ' if(settings.proxy_ip!='') else '')+ settings.proxy_port )
	group.writeEntry( "httpsProxy", settings.proxy_ip+(' ' if(settings.proxy_ip!='') else '')+ settings.proxy_port )
	group.writeEntry( "socksProxy", settings.proxy_ip+(' ' if(settings.proxy_ip!='') else '')+ settings.proxy_port )
	config.sync()
	#Upto this line
	lines=''

	if(settings.apt_proxy_ip):
		line1 = 'Acquire::http::proxy "http://'+settings.username+(':' if(settings.username!='') else '')+settings.password+('@' if(settings.username!='') else '')+settings.apt_proxy_ip+':'+settings.apt_proxy_port+'/";'    
		line2 = 'Acquire::https::proxy "https://'+settings.username+(':' if(settings.username!='') else '')+settings.password+('@' if(settings.username!='') else '')+settings.apt_proxy_ip+':'+settings.apt_proxy_port+'/";'    
		line3 = 'Acquire::ftp::proxy "ftp://'+settings.username+(':' if(settings.username!='') else '')+settings.password+('@' if(settings.username!='') else '')+settings.apt_proxy_ip+':'+settings.apt_proxy_port+'/";'    
		line4 = 'Acquire::socks::proxy "socks://'+settings.username+(':' if(settings.username!='') else '')+settings.password+('@' if(settings.username!='') else '')+settings.apt_proxy_ip+':'+settings.apt_proxy_port+'/";'    
		lines = line1+'\n'+line2+'\n'+line3+'\n'+line4
